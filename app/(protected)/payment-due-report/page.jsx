...
export default function PaymentDueReportPage() {
  ...
  const filterInfo = [
    { label: "Periode", value: formatReportRange(activeExportRange) },
    { label: "Filter", value: activeFilterLabel },
    { label: "Total Data", value: `${filteredRows.length} data` },
  ];

  const reportTitle = `Laporan Jatuh Tempo Pembayaran (${moment(
    activeExportRange.startDate,
  ).format(
    "DD-MM-YYYY",
  )} s/d ${moment(activeExportRange.endDate).format("DD-MM-YYYY")})`;
