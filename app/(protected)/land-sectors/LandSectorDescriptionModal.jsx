...
export default function LandSectorDescriptionModal({ open, onClose, selectedData }) {
    ...
    >
      ...
      >
        <Stack direction="row" spacing={1.5} alignItems="flex-start" sx={{ p: 2.5 }}>
          ...
          >
            <Icon icon="solar:document-text-bold-duotone" fontSize={23} />
          </Box>

          <Box sx={{ minWidth: 0 }}>
            <Typography sx={{ fontWeight: 700, fontSize: 20 }}>Deskripsi Sektor</Typography>
            <Typography sx={{ color: theme.ui.mutedText, fontWeight: 650, fontSize: 12 }}>
              {selectedData?.sector_name || "-"} | {selectedData?.location_name || "-"}
            </Typography>
          </Box>
        </Stack>
        ...
        <Box sx={{ p: 2.5 }}>
          <Typography
            sx={{
              color: selectedData?.description ? "text.primary" : theme.ui.mutedText,
              fontWeight: 650,
              fontSize: 13,
              lineHeight: 1.75,
              whiteSpace: "pre-wrap",
          ...
          >
            {selectedData?.description || "Tidak ada deskripsi untuk sektor ini."}
          </Typography>
        </Box>
