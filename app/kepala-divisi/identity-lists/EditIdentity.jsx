...
const EditIdentity = ({
  ...
  useEffect(() => {
    if (open) {
      // isi field lain seperti biasa
      setNamaLengkap(selectedData?.full_name || "");
      setNomorIndukKependudukan(selectedData?.nik || "");
      setTempatLahir(selectedData?.birth_place || "");
      setTanggalLahir(
        selectedData?.birth_date ? moment(selectedData?.birth_date) : null
      );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.put(
        `/api/identity-list/update-identity`,
        formData
      );
      ...
      onNotify &&
        onNotify({
          ...
          message:
            ...
            "Terjadi error saat memperbarui Data.",
          severity: "error",
        });
      setTimeout(() => {
        loadingFalse();
